class QuizGame {
    ...
    setupSocketListeners() {
        ...
        this.socket.on('showAnswer', (data) => {
            this.showAnswer(data.correctAnswer, data.correctOption, data.answers);
        });
    ...
    showQuestion(question, questionNumber, totalQuestions) {
        this.selectedAnswer = null;
        this.gameStatus.textContent = `Pergunta ${questionNumber} de ${totalQuestions}`;
    ...
    showAnswer(correctAnswer, correctOption, answers) {
        ...
        if (answers[this.playerId] === correctAnswer) {
            this.gameStatus.textContent = `✅ Correto! +${10 + Math.floor(parseInt(this.timeLeftSpan.textContent) * 2)} pontos`;
        } else {
